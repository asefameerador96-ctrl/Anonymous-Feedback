...
const DDL = [
  ...
  // Organisation branding: a logo (stored inline as a data URL) shown beside the
  // Anonvey wordmark to the admin and to respondents taking the survey.
  `ALTER TABLE organizations ADD COLUMN IF NOT EXISTS logo TEXT`,

  // --- Dynamic questionnaire builder ---------------------------------------
  // A survey is a customisable questionnaire belonging to an organisation.
  `CREATE TABLE IF NOT EXISTS surveys (
      id SERIAL PRIMARY KEY,
      org_id INTEGER NOT NULL REFERENCES organizations(id),
      title TEXT NOT NULL DEFAULT 'Employee Survey',
      description TEXT,
      status TEXT NOT NULL DEFAULT 'active',
      collect_manager BOOLEAN NOT NULL DEFAULT TRUE,
  ...
  // Questions belonging to a survey. options is JSON for choice types.
  ...
  // Dynamic responses: answers stored as JSON keyed by question id. Carries an
  // org_id, survey_id and optional manager_id — but NEVER a token or employee
  // reference, preserving anonymity.
  ...
  `CREATE INDEX IF NOT EXISTS idx_sresponses_survey ON survey_responses(survey_id)`,
  `CREATE INDEX IF NOT EXISTS idx_sresponses_org ON survey_responses(org_id)`,

  // A code belongs to a survey.
  `ALTER TABLE invite_tokens ADD COLUMN IF NOT EXISTS survey_id INTEGER`,
];
